...
// let index = 0;
function Game({ index,setIndex ,score, setScore, seconds, startTimer, setGameStarted }) {
  useEffect(() => {
  ...
  const nextBtnPressed = () => {
    if (index < data.length - 1) {
      ...
      alert(`Game Finished! Your Score is ${score} in ${seconds} seconds`);
        ...
        <div className="h-remaining bg-slate-400 flex flex-col bg">
          ...
          <div className="w-full pt-6 text-white font-semibold text-4xl grid place-content-center">
            <div className="bg-[#24242393] p-2  rounded-xl">
              <span className="text-[#fffff3] mx-4">{`Task ${
                index + 1
              }:`}</span>
              <span>{task}</span>
            </div>
          ...
          <div className="p-10 flex gap-[100px]">
            <div
              className={`h-[500px] w-[600px] bg-white flex p-[8px] gap-2 transition-all duration-[1s] ${
                isSelectedCorrect && cssProperty
              } bg-gradient-to-br from-indigo-300 via-pink-300 to-red-300 rounded-lg shadow-2xl`}
            >
              <div
                className={
                  "h-[100px] w-[100px] grid place-content-center bg-[#c7eaf0] rounded-lg shadow-lg"
                }
              >
                1
              </div>
              <div className="h-[100px] w-[100px] grid place-content-center bg-[#c7eaf0] rounded-lg shadow-lg">
                2
              </div>
              <div className="h-[100px] w-[100px] grid place-content-center bg-[#c7eaf0] rounded-lg shadow-lg">
                3
          ...
          <div className="flex w-screen justify-center mt-[-6px]">
            <button
              className="w-[100px] bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-6 py-3 rounded-lg shadow-md hover:opacity-90 hover:cursor-pointer transform transition-transform duration-200"
              onClick={() => {
                nextBtnPressed();
              }}
            >
